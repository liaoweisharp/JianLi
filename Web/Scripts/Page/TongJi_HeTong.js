...
(function () {
    ...
    HTTJ.initData = function () {
        HTTJ.colums = ["ht_Number", "ht_MingCheng", "zanJianLiFeiZongE", "yingShouKuanZongE", "yiShouKuanZongE", "leJiYiKaiPiaoZongE", "jiHuaShouKuanZongE", "shiShouZongE"];
        requireColNameToDiv()
        baseData = {};
        $(".allDiv").show();
    }
    ...
    function getHtml(datas, requireCols) {
        ...
        for (var i = 0; i < requireCols.length; i++) {
            str.push(String.format("<td>{0}</td>", requireCols[i].name));
        }
        str.push("</tr>");
        ...
        for (j = 0; j < requireCols.length; j++) {
            var value = "";
            if (j == 0) {
                if (!requireCols[0].validate || requireCols[0].validate != "money") {
                    value = "合计"
                }
            }
            if (requireCols[j].validate && requireCols[j].validate == "money") {

                value = "<label validate='money'>" + datas.sum(requireCols[j].id); +"</label>";
            }
            str.push(String.format("<td>{0}</td>", value));
        }
        str.push("</tr>")
    ...
    function createJson() {
        ...
        str.push({ itemId: "zhiXingLeiXing", type: "text", title: "执行类型" });
