    ...
    if (context == "getInitData") {
       ...
       /// var baseData["没有合同的前期"]=result[3];//没有合同的前期并且包含自己
        ...
        if(context.type=="review") {
            ...
            var _ht = baseData["heTongArray"].firstOrDefault("ht_Id", htId);
            if (_ht == null) return;
            var itemId="zanJianLiFeiZongE";
            jsonOfJieSuan.push({ title: "暂定监理费总额(万元)", itemId: itemId,type:"text",validate:"money"});
            jieSuan[itemId] = _ht[itemId];
            var itemId = "yingShouKuanZongE";
            jsonOfJieSuan.push({ title: "应收款总额(万元)", itemId: itemId, type: "text", validate: "money" });
            jieSuan[itemId] = _ht[itemId];
            var itemId = "yiShouKuanZongE";
            jsonOfJieSuan.push({ title: "实收款总额(万元)", itemId: itemId, type: "text", validate: "money" });
            jieSuan[itemId] = _ht[itemId];
            var itemId = "leJiYiKaiPiaoZongE";
            jsonOfJieSuan.push({ title: "开票总额(万元)", itemId: itemId, type: "text", validate: "money" });
            jieSuan[itemId] = _ht[itemId];
            var yuE = 0;
            
            if (jieSuan.js_JieSuanJianLiFei) {
                yuE = jieSuan.js_JieSuanJianLiFei - _ht["yiShouKuanZongE"];
            }
            else if (jieSuan.js_YuJieSuanJianLiFei ) {
                yuE = jieSuan.js_YuJieSuanJianLiFei - _ht["yiShouKuanZongE"];
            }
            ...
            var itemId = "yuE";
            jsonOfJieSuan.push({ title: "余额(万元)", itemId: itemId, type: "text", validate: "money" });
            jieSuan[itemId] = yuE;
...
function getHtmlOfHeTong(heTongs){
    ...
    if (heTongs.length > 0) {
        ...
        str.push("<td class='td3'>暂定监理费总额(万元)</td>");
        ...
        for (var i = 0; i < heTongs.length; i++) {
            ...
            str.push(String.format("<td class='td7'>{0}/{1}</td>", ht.isTuiLvYue ? "已退" : "未退", ht.isTuiZhiBao ? "已退" : "未退"));
...
function clickZhiXing(htId) {
    ...
    $.jBox(str, { buttons: { "关闭": "0" }, height: 550, width: 880, top: "3%", title: "合同执行",opacity:0 });
...
function clickDetail(htId) {
    ...
    $.jBox(str, { title: "合同详细", width: 900, height: 600, top: "2%", buttons: { "关闭": "1"},opacity:0 });
    $("#" + id).tabs();
    $invokeWebService_2("~WebService_HeTong.getHeTongInfo", { htId: htId }, null, successCallBack, errorCallBack, null, { userContent: "getHeTongInfo", type: "review", id1: id1, id2: id2, id3: id3, htId: htId });
}
...
function createJson_htv(){
    ...
    jsonArray.push({ itemId: "htv_GongChengZhuangTai", type: "text", title: "工程状态" });
...
function getOptionsFromForm() {
    ...
    var opt = { callback: HT.pageselectCallback, items_per_page: 10, next_text: "下页", num_display_entries:pageSize , num_edge_entries: 2, prev_text: "上页" };
