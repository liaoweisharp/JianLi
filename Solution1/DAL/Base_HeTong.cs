...
namespace DAL
{
    public class Base_HeTong:Base
    {
        ...
        public int countAllHeTong(DAL.CommClass.PageClass pageClass,string where)
        {
            if (where != null)
            {
                return this.dataContext.TabHeTong.Where(ins=>ins.ht_Number.ToString().Contains(where) || ins.ht_MingCheng.Contains(where)).Count();
            }
            else
            {
                return this.dataContext.TabHeTong.Count();
            }
        }
        ...
        public List<DTO.TabHeTong> filterAllHeTong(CommClass.PageClass pageClass,string where,params string[] tabs) {
            ...
            if (where != null)
            {
                return this.dataContext.TabHeTong.Where(ins => ins.ht_Number.ToString().Contains(where) || ins.ht_MingCheng.Contains(where)).OrderByDescending(ins => ins.ht_Id).Skip((pageClass.currentPageNumber) * pageClass.pageSize).Take(pageClass.pageSize).ToList();
            }
            else {
                return this.dataContext.TabHeTong.OrderByDescending(ins => ins.ht_Id).Skip((pageClass.currentPageNumber) * pageClass.pageSize).Take(pageClass.pageSize).ToList();
            }
        }
        ...
        public List<TabHeTong> listHeTongByWhere(DAL.CommClass.QueryWhereHT queryWhereHT)
        {
            ...
            if (queryWhereHT.zhiXingLeiXing != null && queryWhereHT.zhiXingLeiXing.Trim() != String.Empty)
            {
                hetong = hetong.FindAll(ins => ins.TabXiangMuQianQi.qq_ZhiXingLeiXing == queryWhereHT.zhiXingLeiXing.Trim());
            }
