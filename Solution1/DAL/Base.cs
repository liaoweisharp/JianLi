...
namespace DAL
{
    public class Base : IDisposable
    {
        ...
        public Base()
        {
            this.dataContext = new DTO.DataClassesDataContext();
        }
        //~Base()
        //{
        //    Dispose();
        ...
        {
            T ins;
            List<DTO.Tab_RL_User> d = (from p in this.dataContext.Tab_RL_User where p.jl_ChuShengRiQi == DateTime.Now select p).ToList();
            this.dataContext.GetTable<T>().InsertOnSubmit(obj);
            try
            {
                System.Data.Linq.ChangeSet cs = this.dataContext.GetChangeSet();
                this.dataContext.SubmitChanges();//这个有事物的意思，还有回滚。
        ...
        protected List<T> Saves<T>(List<T> objs) where T : class
        {
            ...
            if (objs != null)
            {
                this.dataContext.GetTable<T>().InsertAllOnSubmit<T>(objs);

                try
                {
                    ChangeSet cs = this.dataContext.GetChangeSet();
                    this.dataContext.SubmitChanges();//这个有事物的意思，还有回滚。
        ...
        //        }
        //    }
        //    IQueryable<T> query = from i in this.dataContext.GetTable<T>() where objs.Select(ins => ins[key]).Contains(i[key]) select i;
        //    foreach (T q in objs)
        //    {
